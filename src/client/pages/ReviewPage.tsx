import { Activity, AlertCircle, ArrowLeft, ChevronRight, Play } from "lucide-react";
...
export function ReviewPage({ competitors, providers, mode, preset, notice, canStart, engineReady, onEditModels, onModeChange, onPresetChange, onCompetitorNameChange, onStart }: ReviewPageProps) {
  ...
  return (
    <section className="page-main page-enter" style={{ "--page-width": "980px" } as CSSProperties}>
      <button className="btn btn-ghost is-sm cn-mb-24" onClick={onEditModels}><ArrowLeft /> Edit models</button>
      <div className="section-intro cn-row cn-between cn-gap-32 cn-mb-24">
        <div>
          <h1 className="cn-display">Name your racers.</h1>
      ...
      <div className="panel cn-stack cn-p-8">
        <div className="race-options well cn-bg-well cn-p-16">
          <div className="cn-stack cn-gap-8">
            <span className="cn-label">Run order</span>
          ...
          <div className="cn-stack cn-gap-8">
            <span className="cn-label">Samples</span>
            <div className="segmented">
              {PRESETS.map((option) => (
        ...
        <div className="well cn-bg-well cn-p-8 cn-divide">
          {competitors.map((competitor, index) => (
            <div key={competitor.id} className="review-racer cn-spine" style={{ "--accent": competitor.color } as CSSProperties}>
              ...
              <div className="review-racer-model cn-min-0 cn-stack cn-gap-4"><strong className="cn-truncate">{providerMap.get(competitor.harness)?.models.find((model) => model.id === competitor.model)?.label ?? competitor.model}</strong><span className="cn-meta cn-truncate">{modelLabName(competitor.model, competitor.harness)} · via {HARNESS_LABELS[competitor.harness]}</span></div>
              <label className="review-name cn-stack cn-gap-8"><span className="cn-label">Race name</span><input className="input" value={competitor.label} maxLength={32} onChange={(event) => onCompetitorNameChange(competitor.id, event.target.value)} placeholder="Name this racer" /></label>
            </div>
          ))}
        </div>
        {notice && <div className="banner cn-tone-peach"><AlertCircle size={15} /> {notice}</div>}
        <div className="cn-row cn-end cn-p-8">
          <button className="btn btn-primary is-lg" onClick={onStart} disabled={!canStart || !engineReady}><Play fill="currentColor" /> Start race</button>
        </div>
      </div>
    </section>
  );
