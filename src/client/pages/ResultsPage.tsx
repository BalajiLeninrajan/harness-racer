    ...
    <section className="results-view page-enter">
      ...
      {summary.length > 0 && (
        <div className="results-table panel">
          <div className="panel-heading table-title"><div><Flag size={18} /><h2>Full classification</h2></div><span>{results.filter((result) => result.valid && !result.warmup).length} valid runs{summary.some((row) => row.disqualified) ? ` · ${summary.filter((row) => row.disqualified).length} DSQ` : ""}</span></div>
              ...
              <tbody>
                {summary.map((row) => (
                  <tr className={row.disqualified ? "disqualified" : row.anomalousRuns > 0 ? "has-anomalies" : undefined} key={row.competitor.id}>
                    ...
                    <td data-label="RUNS"><span className="run-count">{row.anomalousRuns > 0 ? `${row.validRuns}/${row.measuredRuns}` : row.measuredRuns}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="results-actions">
        <button className="btn btn-secondary" onClick={onEditGrid}><ArrowLeft size={16} /> Edit grid</button>
        <button className="btn btn-primary" onClick={onRaceAgain}><RotateCcw size={16} /> Race again</button>
      </div>
    </section>
  );
}
