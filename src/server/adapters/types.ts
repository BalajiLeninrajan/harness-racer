 ...
 *    parallel start barrier itself once a lane settles either way, and a
 *    ready status right before an error would be measured as harness prep
 *    for a failed lane.
 ...
 *    opens when the last lane is ready or the first lane ends, ready or
 *    not, and a lane stuck in setup fails on its own setup timer, so a ready
 *    lane only ever waits on lanes that are still on the clock.
